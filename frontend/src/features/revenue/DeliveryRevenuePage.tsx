...
const schema = z.object({
  ...
  gross_amount:     z.coerce.number().positive('المبلغ يجب أن يكون أكبر من صفر'),
...
export default function DeliveryRevenuePage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('تم حفظ إيراد التوصيل')
  ...
  return (
    <PageTransition>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 700 }}>إيرادات التوصيل</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'إجمالي الإيرادات', value: totals.gross, color: 'var(--color-primary)' },
          { label: 'العمولات', value: totals.commission, color: 'var(--color-danger)' },
          { label: 'صافي الإيرادات', value: totals.net, color: 'var(--color-success)' },
        ].map(item => (
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="card" style={{ marginBottom: 24 }}>
          <form onSubmit={handleSubmit(onSubmit)} dir="rtl">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
              <div className="form-field has-value">
                <label style={{ color: 'var(--color-danger)' }}>مبلغ العمولة</label>
                <input readOnly value={commAmount.toFixed(2)} className="form-input amount-field" style={{ color: 'var(--color-danger)' }}/>
              </div>
              <div className="form-field has-value">
                <label style={{ color: 'var(--color-success)' }}>صافي الإيراد</label>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button type="submit" className="btn btn-primary" disabled={isSubmitting}>
                {isSubmitting ? 'جارٍ الحفظ...' : '💾 حفظ الإيراد'}
              </button>
            </div>
          </form>
        </motion.div>
        ...
        {isLoading ? (
          ...
          <table className="data-table">
            ...
            <motion.tbody variants={staggerContainer} initial="initial" animate="animate">
              ...
              {!revenues?.length && (
                <tr><td colSpan={6} style={{ textAlign: 'center', padding: 40, color: 'var(--text-secondary)' }}>لا توجد بيانات</td></tr>
              )}
            </motion.tbody>
          </table>
        )}
