    ...
    <PageTransition>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: 4 }}>تحليل المبيعات والمصروفات والأرباح خلال الفترة المحددة</p>
        </div>
        
        <div className="card" style={{ display: 'flex', gap: 12, padding: '8px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>من</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>إلى</span>
            <input type="date" className="form-input" style={{ padding: '4px 8px', minHeight: 32 }}
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20, marginBottom: 24 }}>
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--text-secondary)' }}>
                ...
                <span style={{ fontWeight: 600 }}>إجمالي الإيرادات</span>
              </div>
            ...
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--text-secondary)' }}>
                ...
                <span style={{ fontWeight: 600 }}>إجمالي المصروفات (شامل التكلفة)</span>
              </div>
              <div style={{ fontSize: 28, fontWeight: 700, fontFamily: 'var(--font-latin)' }}>
            ...
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 12, background: 'linear-gradient(135deg, rgba(212, 168, 83, 0.1) 0%, rgba(212, 168, 83, 0.05) 100%)', border: '1px solid rgba(212,168,83,0.2)' }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 700, fontFamily: 'var(--font-latin)', color: 'var(--color-primary)' }}>
                ...
                <span style={{ fontSize: 13, color: 'var(--text-secondary)', marginRight: 10, display: 'inline-block' }}>
                  الهامش: {((totals?.net_margin ?? 0) * 100).toFixed(1)}%
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="card" style={{ height: 450, padding: '24px 24px 40px 24px', marginBottom: 24 }} dir="ltr">
            <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 24, textAlign: 'right' }}>الأداء المالي عبر الزمن</h3>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trends} margin={{ top: 10, right: 30, left: 30, bottom: 0 }}>
                ...
                />
                <Legend wrapperStyle={{ paddingTop: 20, fontFamily: 'var(--font-arabic)' }} />
                <Area type="monotone" dataKey="revenue" name="الإيرادات" stroke="#34d399" fillOpacity={1} fill="url(#colorRev)" strokeWidth={3} />
                <Area type="monotone" dataKey="expenses" name="المصروفات" stroke="#f87171" fillOpacity={1} fill="url(#colorExp)" strokeWidth={3} />
                <Area type="monotone" dataKey="profit" name="صافي الربح" stroke="#D4A853" fillOpacity={1} fill="url(#colorProf)" strokeWidth={3} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </>
