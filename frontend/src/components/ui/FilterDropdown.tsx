  ...
  return (
    <div style={{ position: 'relative' }}>
      <button
        ...
        onClick={() => setOpen(o => !o)}
        style={{ gap: 6, height: 38, paddingInline: 12 }}
      >
        <span>{selected?.label ?? placeholder ?? t('common.filter')}</span>
        <ChevronDown size={13} style={{ opacity: 0.6, transform: open ? 'rotate(180deg)' : undefined, transition: 'transform 0.2s' }}/>
      </button>
      <AnimatePresence>
        {open && (
          <motion.div
